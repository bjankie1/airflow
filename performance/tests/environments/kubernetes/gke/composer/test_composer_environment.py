...
performance_dag_CONF_FILE_PATH = "performance_dag_conf.json"
...
class TestComposerEnvironment(TestCase):
    ...
    def test_init_performance_dag_config_file(
        self,
        mock_storage_client,
        mock_composer_api,
        ...
        composer = ComposerEnvironment(
            ...
            DAG_FILE_PATH,
            performance_dag_CONF_FILE_PATH,
    ...
    @mock.patch(MODULE_NAME + ".ComposerEnvironment.update_config_body_with_performance_dag_env_variables")
    @mock.patch(MODULE_NAME + ".add_perf_start_date_env_to_conf")
    @mock.patch(MODULE_NAME + ".validate_performance_dag_conf")
    @mock.patch(
    ...
    def test_load_specification(
        ...
        mock_validate_performance_dag_conf,
        mock_add_perf_start_date_env_to_conf,
        mock_update_config_body_with_performance_dag_env_variables,
        ...
        mock_update_config_body_with_performance_dag_env_variables.assert_called_once_with(
            CONFIG_BODY, performance_dag_CONF_FILE_PATH
        )
        mock_add_perf_start_date_env_to_conf.assert_called_once_with(CONFIG_BODY_ENV_VARS)
        mock_validate_performance_dag_conf.assert_called_once_with(CONFIG_BODY_ENV_VARS)
    ...
    def test_load_specification_default_values(
        ...
        mock_update_config_body_with_performance_dag_env_variables.assert_not_called()
        mock_add_perf_start_date_env_to_conf.assert_called_once_with(CONFIG_BODY_ENV_VARS)
        mock_validate_performance_dag_conf.assert_called_once_with(CONFIG_BODY_ENV_VARS)
    ...
    def test_update_config_body_with_performance_dag_env_variables(self, mock_read_json_file):
        mock_read_json_file.return_value = deepcopy(performance_dag_CONF)

        expected_result = deepcopy(CONFIG_BODY_ENV_VARS)
        expected_result.update(performance_dag_CONF)

        self.composer.update_config_body_with_performance_dag_env_variables(
            self.composer.config_body, performance_dag_CONF_FILE_PATH
        )

        mock_read_json_file.assert_called_once_with(performance_dag_CONF_FILE_PATH)

        self.assertEqual(self.composer.get_env_variables(), expected_result)

    @mock.patch(MODULE_NAME + ".read_json_file", return_value=[])
    def test_update_config_body_with_performance_dag_env_variables_not_a_dict(self, _):
    ...
    def test_update_environment_info_force_routing_public_environment(
        ...
        mock_validate_performance_dag_conf.assert_not_called()
    ...
    def test_update_environment_info_reuse_if_exists(
        ...
        mock_validate_performance_dag_conf.assert_called_once_with(new_env_variables)
        self.assertEqual(self.composer.config_body, new_config)
    ...
    @mock.patch(MODULE_NAME + ".generate_copies_of_performance_dag")
    def test_upload_dag_files(
        self, mock_generate_copies_of_performance_dag, mock_get_bucket_name_and_dags_folder
    ):

        self.composer.state = State.UPLOAD_DAG

        mock_generate_copies_of_performance_dag.return_value.__enter__.return_value = (
            "tmp_dir_path",
            [DAG_FILE_PATH],
        )
        ...
        mock_generate_copies_of_performance_dag.assert_called_once_with(DAG_FILE_PATH, CONFIG_BODY_ENV_VARS)
