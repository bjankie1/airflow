...
def get_dags_count(dag_id_prefix: str) -> int:
    """
    ...
    with create_session() as session:
        query = session.query(func.count(DagModel.dag_id)).filter(DagModel.dag_id.startswith(dag_id_prefix))
        dags_count = query.scalar()

    return dags_count


def unpause_dags(dag_id_prefix: str):
    # type: (str) -> None
    """
    ...
    with create_session() as session:
        query = session.query(DagModel).filter(DagModel.dag_id.startswith(dag_id_prefix))
        query.update({DagModel.is_paused: False}, synchronize_session="fetch")


def get_dag_runs_count(dag_id_prefix: str, states: list[str] = None):
    ...
    with create_session() as session:
        query = session.query(func.count(DagRun.dag_id)).filter(DagRun.dag_id.startswith(dag_id_prefix))
...
def collect_dag_run_statistics(dag_id_prefix: str, states: list[str] = None) -> str:
    """
    ...
    dag_run_start_dates = [dag_run.start_date for dag_run in results if dag_run.start_date is not None]
    ...
    test_start_date = timezone.make_naive(timezone.convert_to_utc(min(dag_run_start_dates)))

    dag_run_end_dates = [dag_run.end_date for dag_run in results if dag_run.end_date is not None]
    ...
    task_instance_duration = [
        task_instance.duration for task_instance in task_instances if task_instance.duration is not None
    ]
    ...
    average_task_instance_duration = round(
        sum(task_instance_duration) / len(task_instance_duration) if task_instance_duration else 0.0,
        4,
    )
...
def get_airflow_configuration() -> str:
    """
    ...
    for configuration_option in configuration_options_to_collect:
        ...
        option_name = airflow.configuration.AirflowConfigParser()._env_var_name(section, key)
        ...
        # which will be missing from config of non-composer environment prior to Airflow <1.10.7
            ...
            if section == "core" and key == "store_serialized_dags":
                ...
                option_value = airflow.configuration.get("scheduler", "parsing_processes")
            else:
                raise exception
