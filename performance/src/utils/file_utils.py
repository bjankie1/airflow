...
def read_templated_json_file(
    ...
    rendered_template = json.loads(Template(file_contents).render(**jinja_variables_dict))
...
def check_output_path(output_path: str) -> None:
    ...
    if not os.path.exists(output_path) and not os.path.isdir(os.path.dirname(output_path)):
        raise ValueError(f"Output path '{output_path}' contains a path to non-existing directory.")
